...
export default function DemoModeToggle({ isDemoMode, onToggle }: DemoModeToggleProps) {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.96 }}
      onClick={onToggle}
      className={`fixed bottom-4 left-4 z-50 flex items-center space-x-2.5 px-3.5 py-2 rounded-full border backdrop-blur-xl text-[9px] font-semibold uppercase tracking-[0.2em] transition-all duration-700 shadow-2xl ${
        isDemoMode
